...
浏览器打开 http://127.0.0.1:<端口>
  ...
  - --follow-symlinks：目录软链也进树可读（如 skill 目录里包含软链），防环；默认跳过软链
...
TEXT_EXTS = {
    ...
    ".md", ".mdx", ".mdc", ".json", ".yaml", ".yml", ".toml", ".ini", ".cfg",
...
LANG_MAP = {
    ...
    ".md": "markdown", ".mdx": "markdown", ".mdc": "markdown", ".json": "json", ".yaml": "yaml",
...
<style>
  ...
  * { box-sizing:border-box; margin:0; }
  ...
  /* 不能挂在 chip 内：#entries 和 chip 都会裁剪 overflow。 */
  #entryTooltip { display:none; position:fixed; z-index:9999; max-width:min(480px, calc(100vw - 24px));
                  padding:8px 10px; border-radius:8px; background:#1f2328; color:#fff;
                  font-size:12px; font-weight:400; line-height:1.45; white-space:normal;
                  word-break:break-word; box-shadow:0 8px 24px rgba(0,0,0,.18); pointer-events:none; }
  #entryTooltip.show { display:block; }
...
<div id="entryTooltip" role="tooltip"></div>
...
function showEntryTooltip(btn, x, y){
  ...
  const top = Math.max(8, Math.min(y + 16, window.innerHeight - rect.height - 8));
...
function bindEntryTooltip(btn){
  btn.addEventListener('mouseenter', e => showEntryTooltip(btn, e.clientX, e.clientY));
  btn.addEventListener('mousemove', e => showEntryTooltip(btn, e.clientX, e.clientY));
  btn.addEventListener('mouseleave', hideEntryTooltip);
  btn.addEventListener('focus', () => {
    const r = btn.getBoundingClientRect();
    showEntryTooltip(btn, r.left, r.bottom);
  });
  btn.addEventListener('blur', hideEntryTooltip);
}
  ...
  if (box) {
    if (!shownCommits.length) {
      ...
      box.innerHTML = shownCommits.map(e => {
        ...
        return `<button type="button" class="entry" data-id="${esc(e.id)}" data-full="${esc(full)}" aria-label="${esc(full)}">${esc(label)}</button>`;
      }).join('');
      box.querySelectorAll('.entry').forEach(btn => {
        btn.classList.toggle('on', btn.dataset.id === entryId);
        btn.onclick = () => selectEntry(btn.dataset.id);
        bindEntryTooltip(btn);
      });
    }
  }
...
def main():
    ...
    if "--help" in args or "-h" in args:
        print(
            "usage: serve.py [directory] [port=8770] [--focus relative/path]... "
            "[--follow-symlinks]"
        )
        print("Open a local read-only repository browser on 127.0.0.1.")
        return
    i = 0
    while i < len(args):
        a = args[i]
